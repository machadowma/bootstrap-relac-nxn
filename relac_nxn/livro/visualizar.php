...
try {
  $sth = $dbh->prepare('select * from livro where id = '.$_SESSION["id"].' ');
  $sth->execute();
...
if(isset($_SESSION["id"])) {
  ...
  try {
    ...
    $sth = $dbh->prepare('SELECT a.* FROM livro_autor la LEFT JOIN autor a ON la.id_autor = a.id WHERE la.id_livro = '.$_SESSION["id"].' ');
    $sth->execute();
		$result = $sth->fetchAll(PDO::FETCH_ASSOC);
		...
		if(!empty($result)){
			...
			foreach($result as $row) {
				...
				echo "<td>";
				echo "<a href='relac_excluir.php?id_livro=".$_SESSION["id"]."&id_autor=".$row["id"]."'>";
